...
from database.connections_mdb import active_connection, all_connections, delete_connection, if_active, make_active, \
    make_inactive
from info import ADMINS, LOG_CHANNEL, AUTH_CHANNEL, AUTH_USERS, CUSTOM_FILE_CAPTION, MSG_ALRT, AUTH_GROUPS, \
    P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, SPELL_IMG, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, SUPPORT_GROUP, SUPPORT_CHAT, HOW_DWLD_LINK, DELETE_TIME, \
    SPL_DELETE_TIME
from pyrogram.types import InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery
from pyrogram import Client, filters, enums
from pyrogram.errors import FloodWait, UserIsBlocked, MessageNotModified, PeerIdInvalid
from utils import get_size, is_subscribed, get_poster, search_gagala, temp, get_settings, save_group_settings, \
    get_shortlink, replace_username
...
async def next_page(bot, query):
    ...
    if n_offset == 0:
        ...
        btn.append([InlineKeyboardButton("ᴘᴀɢᴇs", callback_data="pages"),
                    InlineKeyboardButton(f"{math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}",
                                         callback_data="pages"),
...
async def advantage_spoll_choker(bot, query):
    ...
    await query.answer('Ruko Jara Sabar Karo...')
    ...
    if k == False:
        ...
        if files:
            ...
            await bot.send_message(-1001949274353, text=f'<b>SIR THIS MOVIE IS NOT FOUND IN MY DATABASE\n\nMOVIE NAME : {movie}</b>', reply_markup=reply_markup)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('💣 ᴊᴏɪɴ ғᴏʀ ᴜᴘᴅᴀᴛᴇs 💣', url="https://t.me/+UOhabuiyCLVhZGM1")
        ]]
        ...
        buttons = [[
            ...
            InlineKeyboardButton('❄️ ɢᴏᴛ ᴛᴏ ʜᴏᴍᴇ ❄️', callback_data='start')
        ]]
        ...
        buttons = [[
            InlineKeyboardButton('⚙️ ʀᴇᴘᴏʀᴛ ʙᴜɢs & ꜰᴇᴇᴅʙᴀᴄᴋ ⚙️', url=GRP_LNK)
        ], [
            InlineKeyboardButton('📮 ʙᴀᴄᴋᴜᴘ ᴄʜᴀɴɴᴇʟ 📮', url="https://t.me/+UOhabuiyCLVhZGM1")
        ], [
            InlineKeyboardButton('❄️ ʜᴏᴍᴇ ❄️', callback_data='start'),
        ...
        cnlbtn = [[
            InlineKeyboardButton('ᴄʜᴀɴɴᴇʟ', url='https://t.me/+UOhabuiyCLVhZGM1')
        ], [
            InlineKeyboardButton('sᴜᴘᴘᴏʀᴛ', url='https://t.me/+UOhabuiyCLVhZGM1'),
            InlineKeyboardButton('ᴜᴘᴅᴀᴛᴇs', url='https://t.me/+UOhabuiyCLVhZGM1')
        ], [
            InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start")
        ]]
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
                                      ...
                                      reply_markup=InlineKeyboardMarkup(
                                          [[InlineKeyboardButton(text="✅️ Uᴘʟᴏᴀᴅᴇᴅ ✅️", callback_data="doneupld")]])
                                      )
        await client.send_message(SUPPORT_GROUP, text=script.DONE_UPLOAD2,
                                  reply_to_message_id=int(mess_id),
                                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Cᴏᴍᴘʟᴇᴛᴇᴅ Rᴇǫᴜᴇsᴛ ✅️",
                                                                                           url=f"{query.message.link}")]]),
                                  )
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
                                      disable_web_page_preview=True,
                                      reply_markup=InlineKeyboardMarkup(
                                          [[InlineKeyboardButton(text="⚠️ Uɴᴀᴠᴀɪʟᴀʙʟᴇ ⚠️", callback_data="notavl")]])
                                      )
        await client.send_message(SUPPORT_GROUP, text=script.REQ_NO2,
                                  reply_to_message_id=int(mess_id),
                                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Cʜᴇᴄᴋ Yᴏᴜʀ Rᴇǫᴜᴇsᴛ ⚠️",
                                                                                           url=f"{query.message.link}")]]),
                                  )
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
                                      disable_web_page_preview=True,
                                      reply_markup=InlineKeyboardMarkup(
                                          [[InlineKeyboardButton(text="❌ Rᴇᴊᴇᴄᴛᴇᴅ ❌", callback_data="rjctreq")]])
                                      )
        await client.send_message(SUPPORT_GROUP, text=script.REQ_REJECT2,
                                  reply_to_message_id=int(mess_id),
                                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text="Cʜᴇᴄᴋ Yᴏᴜʀ Rᴇǫᴜᴇsᴛ ❌",
                                                                                           url=f"{query.message.link}")]]),
                                  )
        ...
        await query.edit_message_text(text=f"<s>{query.message.text}</s>",
                                      ...
                                      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
                                          text="♻️ Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ ♻️", callback_data="donealrd")]])
                                      )
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            search = re.sub(
                r"\b(please|give|movie|panjabi|full|in|dedo|movies|wali|hd|ep.|-|Movies|malyalam|hello|download|plzz|dubbed|movies|parts|part's|480p|bollywood|hollywood|chinese|vejoo|vejo|film|webseries|tollywood|language|lang|korean|session|sauth|episodes|series|part|complete|parts|the|telgu|1080|720|480|all|hd|4k|720p|1080p|south|episode|episodes|plis|pls|plz|pliz|send|snd|gib|new|latest|bro|malyalamg|gujrati|mallu|thamil|file|link|season|seasons|season's|bhejo|do|in|dubb|dub|dubbed|find|with|subtitle|subtitles|telegu|web|series|s1|s2|s3|s5|s4|s6|s7|s9|s10|in|ful|move|dubing|dubbing|movi|480p|480|webseries|chapter|chapters|s1|s2|s3|s4|s5|s6|s7|s8|s9|s10|links|panjabi|download|ep1|ep2|Download|ep3|ep4|ep5|ep6|ep7|ep8|ep9)",
    ...
    if not spoll:
        ...
        srh_msg = await msg.message.edit_text("<b>Lᴏᴀᴅɪɴɢ Yᴏᴜʀ Rᴇsᴜʟᴛs...🔎</b>")
    ...
    if imdb and imdb.get('poster'):
        try:
            hehe = await message.reply_photo(photo="http://telegra.ph/file/c9a140fe5b9988e8105da.jpg", caption=cap,
                                             reply_markup=InlineKeyboardMarkup(btn))
...
async def advantage_spell_chok(msg):
    ...
    try:
        ...
        logger.exception(e)
        text = mv_rqst.replace(" ", "+")
        button = [[
                   InlineKeyboardButton("Click Here To Check Spelling ✅", url=f"https://www.google.com/search?q={text}")
        ]]
        reply_markup = InlineKeyboardMarkup(button)
        await srh_msg.edit_text(
            text="<b>𝐎𝐩𝐬 𝐌𝐨𝐯𝐢𝐞 𝐍𝐨𝐭 𝐟𝐨𝐮𝐧𝐝 😳 ! \n\n𝐏𝐥𝐞𝐚𝐬𝐞 𝐒𝐞𝐚𝐫𝐜𝐡 𝐥𝐢𝐤𝐞 𝐓𝐡𝐢𝐬 👇\n\nKgf 2 ✅\nKgf 2 hindi ✅\nKgf 2022 ✅</b>"
        )
        await srh_msg.edit_reply_markup(reply_markup)
        await asyncio.sleep(30)
        await srh_msg.delete()
