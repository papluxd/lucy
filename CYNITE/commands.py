...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, RQST_LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, \
    CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, SUPPORT_GROUP
...
async def start(client, message):
    ...
    if len(message.command) != 2:
        buttons = [[
            InlineKeyboardButton('ᴀ🍟 ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ 🍟', url=f'http://t.me/{temp.U_NAME}?startgroup=true')
        ], [
...
async def request(bot, message):
    ...
    rqmsg = await bot.send_message(RQST_LOG_CHANNEL, script.REQUEST_TXT.format(message.text.replace("#request", ""),
                                   ...
                                   reply_markup=InlineKeyboardMarkup(
                                       [[
                                           ...
                                           [
                                               InlineKeyboardButton(text="🉐 Sʜᴏᴡ Oᴘᴛɪᴏɴs 🉐",
                                                                    callback_data=f"morbtn {message.id} {grqmsg.id}")
                                           ]]
                                   )
                                   )
    asyncio.sleep(1.5)
...
async def shortlink(bot, message):
    ...
    if not message.reply_to_message:
        return await message.reply_text(
            f"<b>Hey {message.from_user.mention}, Use this command as a reply to your link shortner api.</b>")
    elif grp_id == None or len(grp_id) < 3:
        return await message.reply_text(
            "<b>Hey, Give me a group id along with the command as a reply to your link shortner api.\n\nFormat: /shortlink -1001*****</b>")
